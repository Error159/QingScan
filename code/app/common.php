...
function dingdingNotice($message, $remote_server = '')
{
    ...
    $remote_server = !empty($remote_server) ? $remote_server : "https://oapi.dingtalk.com/robot/send?access_token=dingdingtoken";
...
function getCode($id, $path)
{
    $path = urlencode($path);
    $baseUrl = "curl --request GET --header 'PRIVATE-TOKEN: gitlabtoken-y' 'https://gitlab-ee.tangqingsong.io/";
    $cmd = "{$baseUrl}api/v4/projects/{$id}/repository/files/{$path}?ref=master'";
