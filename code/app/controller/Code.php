...
class Code extends Common
{
    ...
    {
        Db::table('code')->delete(true);
        for ($i = 1; $i < 10; $i++) {

            $cmd = 'curl --header "PRIVATE-TOKEN: gitlabtoken-y" "https://gitlab-ee.tangqingsong.io/api/v4/projects?simple=true&per_page=100&page=' . $i . '"';
            echo $cmd . PHP_EOL;
            $result = [];
            exec($cmd, $result);
            ...
            foreach ($data as &$value) {
                ...
                $value['hash'] = hash("sha256", $value['id'], false);
            }
            Db::table('code')->insertAll($data);
        }
    ...
    public function scanBitbuket()
    {
        ...
        $codePath = "/root/mycode/work/bitbucket";
    ...
    public function svnScan()
    {
        ...
        while (true) {
            ...
            foreach ($list as $value) {
                ...
                //1. 拉取代码
                if (!file_exists("{$codePath}/{$prName}")) {
                    $cmd = "cd {$codePath}/ && {$value['command']}  --username=qingsong.tang   --password=password";
                    echo $cmd . PHP_EOL;
                    systemLog($cmd);
                }
